using System;
using System.Collections.Generic;
using System.Text;
using System.IO;
using WiiuVcExtractor.Libraries;

namespace WiiuVcExtractor.FileTypes
{
    public class SrlFile
    {
        string path;
        string finalPath;

        public const int startJunkOffset = 0x1000;
        public const int startGameOffset = 0x4000;

        private byte[] game;

        bool verbose;

        public string Path { get { return path; } }
        public string FinalPath { get { return finalPath; } }

        public SrlFile(string srlPath, bool verbose = false)
        {
            path = srlPath;
            finalPath = path.Substring(0, path.Length-3) + "nds";

            using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                using (BinaryReader br = new BinaryReader(fs, new ASCIIEncoding()))
                {
                    game = br.ReadBytes((int)fs.Length);
                }
            }

            this.verbose = verbose;
        }

        public static bool IsSrl(string srlPath)
        {
            using (FileStream fs = new FileStream(srlPath, FileMode.Open,
                FileAccess.Read))
            {
                using (BinaryReader br = new BinaryReader(fs, new ASCIIEncoding()))
                {
                    // Read up to 0xC0
                    br.ReadBytes(0xC0);

                    byte[] nintenLogo = {};

                    // Check Nintendo logo
                    for (int i = 0; i < 156; i++)
                    {
                        if (nintenLogo[i] != br.ReadByte())
                        {
                            return false;
                        }
                    }

                    return true;
                }
            }
        }
    }
}
